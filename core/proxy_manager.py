...
class ProxyManager:
    ...
    def _load_health_data(self):
        ...
        try:
            ...
            if version != self.HEALTH_FILE_VERSION:
                logger.warning(
                    f"Proxy health file version mismatch (expected {self.HEALTH_FILE_VERSION}, got {version}). Ignoring.")
                return
            ...
            logger.info(
                f"Loaded proxy health data: {len(self.proxy_latency)} proxies tracked, "
                f"{len(self.dead_proxies)} dead, {len(self.proxy_cooldowns)} in cooldown")
    ...
    def record_failure(self, proxy_str: str, detected: bool = False, status_code: int = 0):
        ...
        if detected or status_code == 403:
            ...
            logger.error(
                f"[COOLDOWN] Proxy session {self._mask_proxy_key(proxy_key)} detected/403. Cooling down for 1h.")
            ...
            logger.warning(
                f"[COOLDOWN] Proxy session {self._mask_proxy_key(proxy_key)} failed {self.proxy_failures[proxy_key]} times. Cooling down for 5m.")
    ...
    def remove_dead_proxies(self) -> int:
        ...
        if not final_proxies and self.all_proxies:
            ...
            def candidate_score(proxy: Proxy) -> Tuple[float, float]:
                ...
                rep = self.get_proxy_reputation(proxy.to_string()) if getattr(
                    self.settings, "proxy_reputation_enabled", True) else 100.0
            ...
            if best_key in self.proxy_cooldowns:
                logger.warning(
                    "⚠️ All proxies are in cooldown. Temporarily reusing %s to avoid zero-proxy stall.",
        ...
        self.validated_proxies = [p for p in self.validated_proxies if self._proxy_key(
            p) not in current_cooldowns and self._proxy_key(p) not in slow_proxies_keys]

        removed = before_count - len(self.proxies)
        if removed > 0:
            logger.info(
                f"[CLEANUP] Removed {removed} proxies (Dead/Slow). "
                f"Active: {len(self.proxies)} / Total: {len(self.all_proxies)}")
        ...
        if not self.proxies and self.all_proxies:
            ...
            if self.proxy_cooldowns:
                ...
                if restored:
                    ...
                    logger.info(
                        "[RESTORE] Re-enabled one proxy to avoid empty pool: %s",
    ...
    async def validate_proxy(self, proxy: Proxy) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"[TIMEOUT] Proxy {self._mask_proxy_key(self._proxy_key(proxy))} timed out during validation")
            return False
        except Exception as e:
    ...
    async def generate_whitelist_proxies(self, country: str = "all", count: int = 20) -> bool:
        ...
        params_to_try = [
            ...
            {"key": self.api_key, "country": country, "protocol": "http",
                "connection_count": count, "ip_address": current_ip},
                                    ...
                                    if data.get("status") == 1 or data.get("status") == "OK":
                                        ...
                                        if proxy_data:
                                            ...
                                            if isinstance(proxy_str, str):
                                                ...
                                                proxy = self._parse_proxy_string(url)
                                                if proxy:
                                                    logger.info(
                                                        f"[2CAPTCHA] ✅ Fetched proxy config: {proxy.ip}:{proxy.port}")
                                                    return proxy
    ...
    async def fetch_proxies_from_api(self, quantity: int = 10) -> int:
        ...
        logger.info(
            f"Generating {quantity} unique proxies using template from {template_proxy.ip}:{template_proxy.port}...")
    ...
    async def fetch_2captcha_proxies(self, count: int = 100, validate: bool = True,
                                     max_latency_ms: float = 3000) -> int:
        """
        ...
        if not self.proxies:
            ...
            if base_proxy:
                ...
                if loaded == 0:
                    logger.error(
                        "[2CAPTCHA] Failed to fetch proxy config from API and no proxies in file."
                    )
    ...
    def assign_proxies(self, profiles: List[AccountProfile]):
        ...
        if not self.proxies:
            logger.warning(
                "No proxies loaded for provider '%s'. Creating fallback assignments from config.",
                self.proxy_provider)
            return
        ...
        for p in self.proxies:
            ...
            if host_port and host_port in self.proxy_cooldowns and self.proxy_cooldowns[host_port] > now:
                logger.debug(
                    f"Skipping proxy with host in cooldown during assignment: {self._mask_proxy_key(proxy_key)}")
                continue
        ...
        logger.info(
            f"Assigning {len(assignable)} healthy proxies to {len(profiles)} profiles (Sticky Strategy, provider={self.proxy_provider})...")
        ...
        if (
            ...
            if healthy and getattr(self.settings, "proxy_reputation_enabled", True):
                ...
                if len(healthy) < before_filter:
                    logger.debug(
                        f"Filtered out {before_filter - len(healthy)} low-reputation proxies (min score: {min_score})")
            ...
            if not healthy:
                ...
                cooldown_proxies = [(p, self.proxy_cooldowns.get(self._proxy_key(p), 0))
                                    for p in self.proxies if self._proxy_key(p) in self.proxy_cooldowns]
                if cooldown_proxies:
            ...
            if profile.proxy_rotation_strategy == "health_based" and getattr(
    ...
    async def auto_provision_proxies(self, min_threshold: int = 10, provision_count: int = 5) -> int:
        ...
        logger.warning(
            f"LOW PROXY COUNT: {healthy_count}/{min_threshold}. Auto-provisioning {provision_count} proxies...")
    ...
    async def auto_refresh_proxies(
        ...
        if healthy_count >= min_healthy_count:
            logger.info(
                f"[AUTO-REFRESH] ✓ Proxy pool is healthy ({healthy_count} >= {min_healthy_count}). No refresh needed.")
            return True
