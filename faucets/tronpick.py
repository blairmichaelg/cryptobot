"""
TronPick Bot - Optimized for profitability, stealth, and robustness.

This bot implements the TronPick.io faucet with:
- Accurate balance and timer extraction using DataExtractor
- Stealth primitives (human_type, idle_mouse, randomized timing)
- Robust error handling with retries for network/proxy/CAPTCHA failures
...
class TronPickBot(PickFaucetBase):
    """TronPick faucet bot with enhanced stealth and reliability."""
    
    def __init__(self, settings, page: Page, action_lock: Optional[asyncio.Lock] = None):
        """Initialize TronPickBot with enhanced configuration.
        ...
        # TronPick-specific configuration
        self.min_claim_amount = 0.001  # Minimum TRX claim threshold
        self.claim_interval_minutes = 60  # Standard hourly claim
                ...
                try:
                    ...
                    if await element.count() > 0:
                        ...
                        if timer_text and any(c.isdigit() for c in timer_text):
                            # Use DataExtractor for robust parsing
    ...
    async def claim(self) -> ClaimResult:
        """Perform TRX claim with enhanced stealth and error handling.
        ...
        # Navigation with retry (from PickFaucetBase)
        ...
        try:
            ...
            logger.info(f"[{self.faucet_name}] Current balance: {current_balance} TRX")
            ...
            if captcha_present:
                ...
                # Stealth: Human-like delay before solving
            ...
            # Stealth: Human-like click with mouse movement
