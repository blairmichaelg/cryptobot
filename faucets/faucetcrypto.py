            ...
            try:
                ...
                try:
                    ...
                    if any(
                        ...
                        for indicator in ['dashboard', 'home', 'account', 'profile']
                    ):
        ...
        for attempt in range(max_retries):
            try:
                ...
                claim_btn = self.page.locator(
                    ...
                    "button:has-text('Claim Now'), .claim-button"
                )
                ...
                reward_btn = self.page.locator(
                    ...
                    "button:has-text('Collect'), .reward-button"
                )
                if await reward_btn.is_visible(timeout=5000):
                    ...
                    logger.info(
                        f"[{self.faucet_name}] ✅ Claim successful! "
                        f"Balance: {new_balance}, Next claim: {next_claim_min:.1f}min"
                    )
                    return ClaimResult(
                        success=True,
                        status="Claimed",
    ...
    async def withdraw(self) -> ClaimResult:
        ...
        try:
            ...
            crypto_rows = self.page.locator(
                ".crypto-row:has(button.withdraw-btn), "
