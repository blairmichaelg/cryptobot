"""
PolygonPick Bot - Polygon/MATIC faucet from the Pick.io family.

Inherits login from PickFaucetBase and implements MATIC-specific claim logic.
"""
import asyncio
import logging
...
class PolygonPickBot(PickFaucetBase):
    ...
    async def get_balance(self, selector: str = ".balance", fallback_selectors: list = None) -> str:
        """Extract MATIC balance from the page.
                    ...
                    if await element.count() > 0 and await element.first.is_visible():
                        ...
                        if balance_text:
                            ...
                            if balance and balance != "0":
                                logger.debug(f"[{self.faucet_name}] Balance extracted: {balance} MATIC")
                                return balance
    ...
    async def claim(self) -> ClaimResult:
        """Perform MATIC claim with stealth and error handling.
        ...
        try:
            ...
            logger.info(f"[{self.faucet_name}] Current balance: {current_balance} MATIC")
