...
AdBTC is a PTC (Paid-to-Click) site where users earn BTC satoshis by
viewing advertisements ("surf ads").  This module automates the full
lifecycle:
...
class AdBTCBot(FaucetBot):
    ...
    AdBTC is a PTC (Paid-to-Click) site where earnings come
    primarily from viewing surf ads.  This implementation
    includes login, balance checking, ad surfing, and
    ...
    async def is_logged_in(self) -> bool:
        ...
        """
        if "/surf/browse" in self.page.url:
            return True
        balance_sel = (
            ".balance-value, "
            ".nomargbot > div.col.s6.l3.m3.left"
            ".hide-on-small-only > p > b"
        )
    ...
    async def login(self) -> bool:
        ...
        Handles multiple CAPTCHA types (math, hCaptcha) and
        proxy detection.  Automatically strips email aliases
        (+) as AdBTC blocks them.
        ...
        try:
            ...
            await self.page.wait_for_selector(
                'input[name="email"]', timeout=15000,
            )

            # Check for CAPTCHA type selector
            captcha_type_select = self.page.locator(
                "select[name='captcha_type']"
            )
            if (
                await captcha_type_select.count() > 0
                and await captcha_type_select.is_visible()
            ):
                # Switch to hCaptcha if solver is enabled
                if self.solver.api_key:
                    logger.info(
                        f"[{self.faucet_name}] "
                        "Switching to hCaptcha..."
                    )
                    await captcha_type_select.select_option(
                        "1",
                    )
                    await self.random_delay(0.5, 1.0)

            # Handle Math Captcha if active
            math_img = self.page.locator(
                "img[src*='captcha.php']"
            )
            if (
                await math_img.count() > 0
                and await math_img.is_visible()
            ):
                logger.info(
                    f"[{self.faucet_name}] "
                    "Math CAPTCHA detected, solving..."
                )
                await self.solve_math_captcha()

            # AdBTC blocks email aliases with '+'
            ...
            logger.info(
                f"[{self.faucet_name}] "
                "Checking for hCaptcha/Turnstile..."
            )
            ...
            if (
                ...
                and "detected" in content.lower()
            ):
                logger.error(
                    f"[{self.faucet_name}] "
                    "Proxy detected - login blocked."
                )
                return False
    ...
    async def claim(self) -> ClaimResult:
        ...
        AdBTC's main earning method is surfing ads, not a
        traditional faucet.  This method validates login and
        extracts balance.

        Returns:
            ClaimResult with success status and next claim
            timing.
        """
        try:
            logger.info(
            ...
            if balance == "0":
                logger.warning(
                    ...
                    "extract balance, using fallback "
                    "auto-detection"
                )
                balance = await self.get_balance(
                    balance_selectors[0],
            ...
            for selector in timer_selectors:
                try:
                    ...
                    if (
                        ...
                        and await timer_el.first
                        .is_visible()
                    ):
                        timer_minutes = (
                            await self.get_timer(selector)
                        )
                        if timer_minutes > 0:
                            logger.info(
                                f"[{self.faucet_name}] "
            ...
            logger.info(
                f"[{self.faucet_name}] Logged in "
                f"successfully. Balance: {balance}"
            )
    ...
    def get_jobs(self) -> List[Any]:
        """Return AdBTC-specific jobs for the scheduler.

        Returns:
            List of Job objects for claim, withdrawal, and
            surf ads.
        """
        ...
        # Job 3: Surf Ads (main earning method)
        jobs.append(Job(
            priority=2,
    ...
    async def view_ptc_ads(self) -> None:
        ...
        Implements robust error handling and stealth features.
        """
        try:
            logger.info(
                f"[{self.faucet_name}] "
            ...
            for attempt in range(max_ads):
                try:
                    ...
                    # Check for CAPTCHA on Surf Page
                    captcha_frame = self.page.locator(
                        ".h-captcha, "
                        "iframe[src*='hcaptcha'], "
                    ...
                    # Check for "Click to start" button
                    start_btn = self.page.locator(
                        "input[value='Click to start'], "
                    ...
                    if await open_btn.count() == 0:
                        logger.info(
                            ...
                            f"Viewed {ads_viewed} ads."
                        )
                        break

                    logger.info(
                        f"[{self.faucet_name}] "
                        f"Opening ad {ads_viewed + 1}..."
                    )
                    ...
                    # Verify we're on surf page
                    current_url = self.page.url
                    if "/surf/browse" not in current_url:
                        logger.info(
                            f"[{self.faucet_name}] "
                            "Navigating back to surf "
                            "page..."
                        )
                        await self.page.goto(
                            f"{self.base_url}/surf/browse",
                    ...
                    logger.error(
                        ...
                        "processing ad "
                        f"{attempt + 1}: {ad_error}"
                    )
                    continue

            logger.info(
                f"[{self.faucet_name}] Surf ads session "
                "complete. Total ads viewed: "
                f"{ads_viewed}"
            )
    ...
    async def withdraw(self) -> ClaimResult:
        ...
        Handles FaucetPay withdrawal with password verification
        and confirmation.  Implements retry logic for network
        timeouts and CAPTCHA failures.

        Returns:
            ClaimResult with success status and next withdrawal
            timing.
        """
        try:
            logger.info(
            ...
            await self.page.goto(
                f"{self.base_url}/index/withdraw",
                timeout=nav_timeout,
            )
            ...
            # Select FaucetPay (lower thresholds)
            faucetpay_btn = self.page.locator(
                "a:has-text('To FaucetPay'), "
                "button:has-text('To FaucetPay')"
            )

            if await faucetpay_btn.count() == 0:
                logger.info(
                    f"[{self.faucet_name}] FaucetPay "
            ...
            withdraw_confirm = self.page.locator(
            ...
            # Some accounts require a password
            pass_field = self.page.locator(
                "input[name='password'], "
                "input[type='password']"
            )
            if (
                await pass_field.count() > 0
                and await pass_field.is_visible()
            ):
                logger.info(
                    f"[{self.faucet_name}] Password "
                    "required for withdrawal, filling..."
                )
                creds = self.get_credentials("adbtc")
                if creds and creds.get('password'):
                    ...
                    logger.error(
                        f"[{self.faucet_name}] No password "
                        "available for withdrawal."
                    )
                    return ClaimResult(
                        success=False,
                        status=(
                            "No Password for Withdrawal"
                        ),
                        next_claim_minutes=1440,
                    )
            ...
            try:
                ...
                logger.debug(
                    f"[{self.faucet_name}] Post-submit "
                    f"wait timed out: {e}"
                )
            ...
            if any(
                ...
                logger.warning(
                    f"[{self.faucet_name}] Withdrawal "
                    "failed - check balance/threshold."
                )
                return ClaimResult(
                    success=False,
                    status=(
            ...
            # Uncertain outcome
            logger.warning(
                f"[{self.faucet_name}] Withdrawal outcome "
                "unclear, manual verification needed."
            )
            timestamp = asyncio.get_event_loop().time()
            await self.page.screenshot(
                path=(
                    f"adbtc_withdrawal_{timestamp}.png"
                ),
            )
    ...
    async def solve_math_captcha(self) -> bool:
        """Extract and solve simple AdBTC math captchas.

        AdBTC uses basic arithmetic captchas (e.g. '7 + 3') as
        an alternative to hCaptcha.  This method extracts the
        equation from page content and solves it.
        ...
        try:
            ...
            if not match:
                logger.debug(
                    f"[{self.faucet_name}] No math captcha "
            ...
            logger.info(
                f"[{self.faucet_name}] Solved Math "
                f"Captcha: {num1} {op} {num2} = {result}"
            )

            # Fill in the answer
            answer_field = self.page.locator(
                'input[name="number"], '
                'input[type="text"][name*="captcha"]'
            )
            if await answer_field.count() > 0:
                await self.human_type(
                    answer_field, str(result),
                )
                await self.random_delay(0.3, 0.6)
                return True

            logger.warning(
                f"[{self.faucet_name}] Math captcha "
