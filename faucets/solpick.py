"""
SolPick Bot - Solana faucet from the Pick.io family.

Inherits login from PickFaucetBase and implements SOL-specific claim logic.
"""
import asyncio
import logging
...
class SolPickBot(PickFaucetBase):
    ...
    async def get_balance(self, selector: str = ".balance", fallback_selectors: list = None) -> str:
        """Extract SOL balance from the page.
                    ...
                    if await element.count() > 0 and await element.first.is_visible():
                        ...
                        if balance_text:
                            ...
                            if balance and balance != "0":
                                logger.debug(f"[{self.faucet_name}] Balance extracted: {balance} SOL")
                                return balance
    ...
    async def claim(self) -> ClaimResult:
        """Perform SOL claim with stealth and error handling.
        ...
        try:
            ...
            logger.info(f"[{self.faucet_name}] Current balance: {current_balance} SOL")
